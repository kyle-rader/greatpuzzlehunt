...
Meteor.methods({
  ...
  'promo_codes.register'(data) {
    ...
    const code = PromoCodes.findOne({ code: data.promocode });
    if (!code) throw new Meteor.Error(400, 'That promo code is not valid');

    // 2. Register User
    const transaction = { _id: code.code };
    const txResult = Transactions.upsert(transaction, { $set: { promocode: code.code } });
