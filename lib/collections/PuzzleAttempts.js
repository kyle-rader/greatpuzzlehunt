...
Meteor.methods({
    ...
    startPuzzle: function(fields) {
        ...
        // Setup a timeout to check back in an hour for Time Wall
        if (Meteor.isServer) {
            
            Meteor.setTimeout(() => {

                let puzzleAttempt = PuzzleAttempts.findOne({_id: attemptId});
                if (!puzzleAttempt)
                    throw new Meteor.Error(500, `The puzzleAttempt ${attemptId} has been lost!`);

                // No Finish Time at this moment in time Means a final Puzzle Score of 2 hours.
                if (!puzzleAttempt.finishTime) {

                    // We have run into the Time Limit! Set the end time to now + 1 hour.
                    PuzzleAttempts.update({_id: attemptId}, {$set: {
                        finishTime: new Date(),
                        finalScore: (2 * ONE_HOUR)
                    }});

                    // TODO:
                    // Set Team's new destination
                }
            }, ONE_HOUR);
        }
    },
    ...
    submitAnswer(fields) {
        ...
        console.log('Answer submitted', fields);
