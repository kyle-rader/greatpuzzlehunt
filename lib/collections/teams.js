...
import { requireUser, requireVolunteer, NonEmptyString, requirePattern, notDuringGameplay } from '../imports/method-helpers.js';
import { pick, find, findIndex, sample } from 'lodash';
...
Meteor.methods({
  ...
  'team.begin'() {
    ...
    const puzzles = Puzzles.find({ stage: 0 }).fetch();
    if (puzzles.length === 0) throw new Meteor.Error(400, "The puzzles aren't ready yet");
    return Teams.update(user.teamId, {
      $set: {
        hasBegun: true,
        startLocation: sample(puzzles).location,
      },
    });
  },
