...
function buildUsersAndTeams() {
  ...
  const usersWithTeams = users.map((user) => {
    if (user.teamId) {
      user.teamName = teams[user.teamId].name;
      user.teamDivision = teams[user.teamId].division;
    }
    return user;
  });

  const fields = ["createdAt", "firstname", "lastname", "email", "accountType", "paid", "ticketUsed", "lookingForTeam", "age", "city", "state", "address", "zip", "phone", "ecName", "ecRelationship", "ecPhone", "ecEmail", "photoPermission", "holdHarmless", 'teamName', 'teamDivision'];
  let csv = fields.join(",") + "\n";
